...
class Solution
{
    ...
    static int[] dijkstra(int V, ArrayList<ArrayList<ArrayList<Integer>>> adj, int S)
    {
        ...
        //using set which is optimized than priorityQueue<>...
        ...
        while(!set.isEmpty())
        {
            ...
            for(ArrayList<Integer>li:adj.get(node))
            {
                ...
                if(yy+dis<arr[xx])
                {
                    // the less distance existed so now we need to remove more distance....
                    if(arr[xx]!=Integer.MAX_VALUE)
                    {
                     Pair pp=new Pair(arr[xx],xx);
                     //remove the pair 
                     set.remove(pp);
                    }
                    
                    arr[xx]=yy+dis;
                    set.add(new Pair(yy+dis,xx));
                }
            }
        }
        return arr;
